...
most expressed genes, and the DE with the other clusters.

TODO
```{r eval=FALSE, include=FALSE}
# subset only the cluster of interest
sce_10 <- sce[ ,sce$cluster_k20 == 10]
...
# save result
write.table(top_genes_10, file = here("outs", age, "top_genes_10k20_02.tsv"), sep = "\t", quote = FALSE, col.names = FALSE, row.names = TRUE)
```

We perform a DE between  each cluster and the other clusters to pull out
more interesting genes from each cluster, this also yields ribosomal
genes and genes that do not seem specific to any particular celltype. We
will have a closer look in the cluster QC.

```{r eval=FALSE, include=FALSE}
# compute markers
if(!file.exists(here("outs", age, "markers_k20_01.RDS"))){
  markers <- findMarkers(sce, groups = sce$cluster_k20, direction="up", lfc=1)
  saveRDS(markers, here("outs", age, "markers_k20_01.RDS"))
}else{
  markers <- readRDS(here("outs", age, "markers_k20_01.RDS"))
}
# select cluster 10
markers_10 <- markers[["10"]]
top_markers <- head(markers_10, 50)
write.csv(top_markers, here("outs", age, "top_markers_10k20_01.csv"), quote = FALSE)
```
...
In one line:
"Neuron","Endothelial-Pericytes","Endothelial-Pericytes ","Immune-Monocytes","Astrocyte-Oligo","Immune-Microglia-Macrophages","Endothelial-Pericytes","Immune","ChP-Epithelial","OPC","Astrocyte","Oligo","Astrocyte","Astrocyte","Oligo","Immune","Immune","Neuron","Immune-BAM","Pvalb-Astrocyte","Immune","Cldn5-Astrocyte-Plp1"
