...
The cluster names are not updated! TODO
...
The neighboorghoods with strong down regulation are the microglia. 
...
nh_graph_tsne_pl <- plotNhoodGraphDA(milo, da_results, layout="TSNE",alpha=0.1) 
...
While neighbourhoods tend to be homogeneous, we can define a threshold for `cluster_k10_fraction` to exclude neighbourhoods that are a mix of cell types.

```{r}
ggplot(da_results, aes(cluster_oligo_k10_fraction)) + geom_histogram(bins=50)
```
```{r}
da_results$cluster_oligo_k10 <- ifelse(da_results$cluster_oligo_k10_fraction < 0.5, "Mixed", da_results$cluster_oligo_k10)
```
...
plotReducedDim(milo, dimred = "TSNE", colour_by="cluster_oligo_k10", text_by = "cluster_oligo_k10", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")
