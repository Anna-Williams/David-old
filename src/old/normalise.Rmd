...
between samples 
[(Lun et al., 2016)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4848819/)

```{r norm}
# Only compute if first time
if (!(file.exists(here("processed", age,  "sce_norm.RDS")))) {
  sce <- readRDS(here("processed", age, "sce_QC.RDS"))
  # For reproducibility
  set.seed(100)
  ...
  saveRDS(sce, here("processed", age,  "sce_norm.RDS"))
} else{
  sce <- readRDS(here("processed", age,  "sce_norm.RDS"))
}
```

On top of normalisation the data is also log-transformed. The 
log-transformation is useful as differences in the log-values represent log-fold
changes in expression. Or in other words, which is more interesting - a gene
that is expressed at an average count of 50 in cell type A and 10 in cell type
B, or a gene that is expressed at an average count of 1100 in A and 1000 in B? 
...
SingleCellExperiment object. We can then use this to determine which experimental
factors are contributing most to the variance in expression. This is useful for 
diagnosing batch effects or to quickly verify that a treatment has an effect.
...
# Before normalisation
# Only compute if first time
if (!(file.exists(here("processed", age,  "variance_explained.RDS")))) {
  ...
  saveRDS(var, here("processed", age,  "variance_explained.RDS"))
  #If not just load created object
} else {
  var <- readRDS(here("processed", age,  "variance_explained.RDS"))
}
...
```{r var_norm}
# After normalisation
if (!(file.exists(here("processed", age,  "variance_explained_norm.RDS")
))) {
  ...
  saveRDS(var_norm, here("processed", age,  "variance_explained_norm.RDS"))
} else{
  var_norm <- readRDS(here("processed", age,  "variance_explained_norm.RDS"))
}
plotExplanatoryVariables(var_norm)

# Again after normalisation but other parametres
if (!(file.exists(
  here("processed", age,  "variance_explained_norm_2.RDS")
))) {
  ...
  saveRDS(var_norm_2, here("processed", age,  "variance_explained_norm_2.RDS"))
} else{
  var_norm_2 <- readRDS(here("processed", age,  "variance_explained_norm_2.RDS"))
}
...
Another way to assess the variance is with a PCA plot. 
Here again we can see how the sequencing depth(sum) explains most 
of the variance before the normalisation
...
```{r batch-correct}
if (!(file.exists(
  here("processed", age,  "sce_corrected.RDS")
))) {
set.seed(100)
sce <- correctExperiments(sce,
  ...
  saveRDS(sce, here("processed", age,  "sce_corrected.RDS"))
} else {
...
```{r}
# Only compute if first time
if (!(file.exists(here("processed", age,  "variance_explained_corrected.RDS")))) {
  ...
  saveRDS(var_corrected, here("processed", age,  "variance_explained_corrected.RDS"))
  #If not just load created object
} else{
  var_corrected <- readRDS(here("processed", age,  "variance_explained_corrected.RDS"))
}
