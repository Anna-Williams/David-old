...
We want to make sure that the differences in the quality are not due to the fact
the mice are KO before deleting the clusters
...
We consider that if cluster are formed by more than `r ko_pct` % cells from the
KO, the clustesr might be different due to the biological condition and it is
better to not remove them for now even if they have high mitochondrial genes or
low umi counts.
...
The "protected" clusters are

```{r}
protected <- unique(sce$cluster_k5[sce$cluster_k5 %in% c( high_mt_clusters, low_umi_clusters) &
