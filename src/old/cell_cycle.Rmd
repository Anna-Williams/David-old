...
The plots are saved in "outs/`r age`/Cyclins"

```{r fig.height=7, include=FALSE}
ccn_genes <- rownames(sce)[grep("^Ccn[abde][0-9]$", rownames(sce))]
lapply(levels(sce$clusters_named),
            function(cluster_named){
              pdf(here("outs", age, "Cyclins", paste0(cluster_named,".pdf")))
             plotHeatmap(sce,
            columns =sce$clusters_named == cluster_named,
            order_columns_by=c("genotype"),
            colour_columns_by = c("genotype"),
            cluster_rows=FALSE, 
            cluster_cols = FALSE,
            features= sort(ccn_genes),
            main = cluster_named)
            dev.off()
             }
            ) 

pdf(here("outs", age, "Cyclins", "all_clusters_named.pdf"))
plotHeatmap(sce,
            #columns =sce$clusters_named == cluster_named,
            order_columns_by=c("clusters_named"),
            colour_columns_by = c("clusters_named"),
            cluster_rows=FALSE, 
            cluster_cols = FALSE,
            features= sort(ccn_genes)
            )
...
pdf(here("outs", age, "Cyclins", "all_genotype.pdf"))
plotHeatmap(sce,
            #columns =sce$clusters_named == cluster_named,
            order_columns_by=c("genotype"),
            colour_columns_by = c("genotype"),
...
```{r}
freq_WT.KO_G1.G2M.S <- table(cycle=cycle_cells$pruned.labels, genotype=sce$genotype)
chisq.test(freq_WT.KO_G1.G2M.S)
```

We can also perform the same test in each one of our cell types. Due to the presence of small counts (expected frequencies \<5) we use the fisher's exact test, as advised in [(Hae-Young Kim et. al.)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5426219/)
