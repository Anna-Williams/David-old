...
library(PCAtools) # find elbow point
```
...
```{r}
elbow <- PCAtools::findElbowPoint(pct_var)
```
```{r}
horn <- PCAtools::parallelPCA(logcounts(sce)[hvgs,],
    BSPARAM=BiocSingular::IrlbaParam(), niters=10)
horn$n
```

I tried to use some of the PCAtools features to estimate
the number of PCs, such as the elbow and  Horn's parallel analysis, but both yeild a result lower than 15 PCs. These 
methods tend to underestimate the number of PCs to use, and 
it is remonded using between 20 and 40 PCs. 
