...
This computes for each cell some useful metrics such as the number of 
...
summary_outlier <- data.frame(
  lib_size_high = c(sum(outlier_lib_high),attr(outlier_lib_high, "thresholds")[2]),
  expression_high = c(sum(outlier_expr_high), attr(outlier_expr_high, "thresholds")[2]),
  lib_size_low = c(sum(outlier_lib_low),attr(outlier_lib_low, "thresholds")[1]),
  expression_low = c(sum(outlier_expr_low), attr(outlier_expr_low, "thresholds")[1]),
...
write.csv(summary_outlier, here("outs", "autofilter_summary.csv"))
# Add if it is an outlier to the metadata
sce$outlier <- outlier
}else{
  summary_outlier <- read.csv(here("outs", "autofilter_summary.csv"))
}
summary_outlier
```
...
This measures the number of detected genes per cell dividided by its lirary size. This will be very useful to delete  the cells that have low
...
cells, might be a bit too harsh. This could be due to the fact the
distribution was not normal. We could start with
25,000 instead of `r round(summary_outlier$lib_size_high[2])`.

The other thresholds seem more reasonable. These
are deleting cells with less than `r round(summary_outlier$expression_high[2])` or more than 
`r round(summary_outlier$expression_low[2])` detected genes, and less than `r round(summary_outlier$lib_size_low[2])` umi counts.
...
mitochondrial threshold, `r round(summary_outlier$mt_pct[2], 2)` %, this is a bit high compared with other datasets but
it is already deleting `r summary_outlier$mt_pct[1]` cells. Lower down to only 10 % would delete `r sum(sce$subsets_mt_percent>10)` cells. 
...
summary_discard <- data.frame(
  ...
  expression_high = c(sum(discard_expr_high), attr(outlier_expr_high, "thresholds")[2]),
  lib_size_low = c(sum(discard_lib_low),attr(outlier_lib_low, "thresholds")[1]),
  expression_low = c(sum(discard_expr_low), attr(outlier_expr_low, "thresholds")[1]),
...
write.csv(summary_discard, here("outs", "reason_for_discard.csv"))
...
saveRDS(sce, here("processed", "sce_old_preliminary.RDS"))}else{

}
summary_discard <- read.csv(here("outs", "reason_for_discard.csv"))
summary_discard
...
The object has `r dim(sce)[1]` genes and `r sum(sce$discard == FALSE)` cells after filtering
