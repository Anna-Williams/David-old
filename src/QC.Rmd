...
outlier_lib <- isOutlier(sce$total, log = TRUE, type = "both")
outlier_expr <- isOutlier(sce$sum, log = TRUE, type = "both")
...
DataFrame(
  lib_size = sum(outlier_lib),
  expression = sum(outlier_expr),
  mt_pct = sum(outlier_mt),
  total = sum(outlier)
)
# And the thresholds

attr(outlier_lib, "thresholds")
attr(outlier_expr, "thresholds")
...
### PCA
 ...
 It is usefull to visualize the QC parametres.  
```{r PCA}
