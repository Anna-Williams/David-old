...
Diagnostic plots to visualize the data distribution. The orange cells are marked as outliers by the automatic
detection from scater. The thresholds are not very useful, so these will poblably be ignored. 
...
This measures will probably very useful to get rid of these cells that have low gene counts but a relatively high umi count.
The isoutlier function did not work very well with the sum of umi and the detected 
genes. This function expects roughly a normal distribution. Bellow we try with the ratio
between these two parametres, that do follow a normal distribution. 
```{r}
if(!file.exists(here("processed", "sce_preliminary.RDS"))){
...
```
The suggestions here are very reasonable, it is not enough to delete all the cells we do not 
trust but we can definitely take this into consideration as a threshold parametre. 

<details>
  <summary>RE-do same plots as above with the ratio outlier marked</summary>
